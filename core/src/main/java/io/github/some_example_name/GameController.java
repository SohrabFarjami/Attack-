...
public class GameController {
	...
	public void defend() {
		...
		if (attackerCards.size != defenderCards.size) {
			...
			warn(String.format("Attack has %d cards and defence has %d cards %n", attackerCards.size,
					defenderCards.size));
			return;
		}
	...
	private boolean validDefence() {
		...
		for (int i = 0; i < attackHand.size; i++) {
			...
			if (attackCard.getSuit() != defendCard.getSuit()
				...
				warn(String.format("%s does not beat %s (Wrong Suit) %n", defendCard, attackCard));
				return false;
			}
			if (defendCard.getValue() < attackCard.getValue()
					&& defendCard.getSuit() == attackCard.getSuit()) {
				warn(String.format("%s does not beat %s (Smaller Value) %n", defendCard, attackCard));
			...
			if (defendCard.getSuit() == trumpCard.getSuit()
					&& attackCard.getSuit() == trumpCard.getSuit()) {
				if (defendCard.getValue() < attackCard.getValue()) {
					warn(String.format("%s does not beat %s (Smaller Value) %n", defendCard,
							attackCard));
					return false;
				}
			}
